...
*       записать в специальный адрес в памяти, чтобы это могло быть записано в
*       память этого самого девайса.
*       Чтобы работать с дейвасом, нам достаточно знать что в нем есть некий
*       контролирующий чип, который делает всю работу грязную работу за нас и
*       позволяет процессору взаимодействовать с напрямую с железом дейваса.
*       У такого чипа есть регистры в которые можно что-то записывать или что-то
*       из них читать, и значение этих регистров указывет чипу что делать.
...
unsigned char   port_byte_in(unsigned short port)
{
    ...
    /* unsigned short port: регистр какого-либо девайса, из которого мы */
    /* что-то прочтем. */
