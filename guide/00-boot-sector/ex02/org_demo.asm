; ======================== BOOT SECTOR / ex02 / org_demo =======================
; Простая программа загрузочного сектора (boot sector), которая демонстрирует
; работу адресов, используя директиву [org <адрес>]
; ------------------------------------------------------------------------------
; Мы выяснили в bs02, что чтобы процессор корректно нашел объявленное нами
; значение, нужно добавлять 0x7c00 к адресу этого значения (т.е. "сместить"
; адрес на 0x7c00, англ. offset - смещение). Это довольно неудобно, и поэтому
; ассемблер позволяет нам указать "глобальное смещение" с помощью директивы
; [org <адрес>]. Посмотрим как это работает.
; ------------------------------------------------------------------------------
...
mov al, '1'				; Выводим номер способа
int 0x10
